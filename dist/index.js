...
var require_github2 = __commonJS({
  "src/github.js"(exports2, module2) {
    ...
    async function createStatusCheck2(
      repoToken,
      markupData,
      conclusion,
      reportName2
      ...
      try {
        ...
        const response = await octokit.rest.checks.create({
          ...
          name: `${reportName2.toLowerCase()}_${runId}`,
          head_sha: git_sha,
          status: "completed",
          conclusion,
          output: {
    ...
    async function lookForExistingComment(octokit) {
      ...
      while (hasMoreComments) {
        ...
        if (commentsResponse.status == 200) {
          if (commentsResponse.data) {
            ...
            if (existingComment) {
              core2.info(
                `An existing postman test results comment (${existingComment.id}) was found and will be udpated.`
              );
      ...
      core2.info(
        "An existing postman test results comment was not found, will create a new one instead."
      );
      return null;
    }
    async function createPrComment2(
      repoToken,
      markupData,
      updateCommentIfOneExists2
    ) {
      try {
...
${markupData}`,
        ...
        if (success) {
          ...
          core2.setFailed(
            `Failed to ${action} PR comment. Error code: ${response.status}.`
          );
        }
      } catch (error) {
        core2.setFailed(
...
var userMessage = core.getInput("message", requiredArgOptions);
var totalTests = core.getInput("total-tests", requiredArgOptions);
var failedTests = core.getInput("failed-tests", requiredArgOptions);
var reportName = core.getInput("status-name", requiredArgOptions);
